...
@pytest.mark.asyncio
async def test_list_group_expenses_success(expense_service, mock_group_data, mock_expense_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.expenses.find.return_value.sort.return_value.skip.return_value.limit.return_value = mock_expense_cursor
        mock_db.expenses.count_documents = AsyncMock(return_value=1)

        # Mock aggregation for summary
        mock_aggregate_cursor = AsyncMock()
        mock_aggregate_cursor.to_list.return_value = [{"totalAmount": 100.0, "expenseCount": 1, "avgExpense": 100.0}]
        mock_db.expenses.aggregate.return_value = mock_aggregate_cursor

        with patch.object(expense_service, '_expense_doc_to_response', new_callable=AsyncMock) as mock_response:
            mock_response.return_value = {"id": "expense_id", "description": "Test Dinner"}

            result = await expense_service.list_group_expenses("65f1a2b3c4d5e6f7a8b9c0d0", "user_a")
...
@pytest.mark.asyncio
async def test_list_group_expenses_pagination(expense_service, mock_group_data, mock_expense_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        expenses_page_2 = [mock_expense_data, mock_expense_data] # Dummy data for page 2
        ...
        mock_aggregate_cursor = AsyncMock()
        mock_aggregate_cursor.to_list.return_value = [{"totalAmount": 200.0, "expenseCount": 2, "avgExpense": 100.0}]
        mock_db.expenses.aggregate.return_value = mock_aggregate_cursor

        with patch.object(expense_service, '_expense_doc_to_response', new_callable=AsyncMock) as mock_response:
            # Each call to _expense_doc_to_response will return a unique dict to simulate different expenses
            mock_response.side_effect = [{"id": "expense_1", "description": "Dinner 1"}, {"id": "expense_2", "description": "Dinner 2"}]

            result = await expense_service.list_group_expenses("65f1a2b3c4d5e6f7a8b9c0d0", "user_a", page=2, limit=2)
            ...
            mock_db.expenses.find.return_value.sort.return_value.skip.assert_called_with(2)
            mock_db.expenses.find.return_value.sort.return_value.skip.return_value.limit.assert_called_with(2)


@pytest.mark.asyncio
async def test_list_group_expenses_filters(expense_service, mock_group_data, mock_expense_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        with patch.object(expense_service, '_expense_doc_to_response', new_callable=AsyncMock) as mock_response:
            mock_response.return_value = {"id": "expense_id", "description": "Filtered Dinner"}
...
async def test_list_group_expenses_group_not_found(expense_service):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        with pytest.raises(ValueError, match="Group not found or user not a member"):
            await expense_service.list_group_expenses(valid_but_non_existent_group_id, "user_a")
...
async def test_delete_expense_success(expense_service, mock_expense_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.settlements.delete_many = AsyncMock(return_value=mock_delete_settlements_result)
        ...
        mock_db.settlements.delete_many.assert_called_once_with({"expenseId": expense_id})
        mock_db.expenses.delete_one.assert_called_once_with({"_id": ObjectId(expense_id)})
...
async def test_delete_expense_not_found(expense_service):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.settlements.delete_many = AsyncMock() # Should not be called if expense not found
...
async def test_delete_expense_failed_deletion(expense_service, mock_expense_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.settlements.delete_many.assert_called_once() # Settlements should still be attempted to be deleted
...
async def test_create_manual_settlement_success(expense_service, mock_group_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        def sync_mock_user_find_cursor_factory(query, *args, **kwargs):
            ...
            cursor_mock.to_list = AsyncMock(return_value=users_to_return) # .to_list() is an async method on the cursor
        ...
        result = await expense_service.create_manual_settlement(group_id, settlement_request, user_id)
        ...
        assert inserted_doc["expenseId"] is None # Manual settlements have no expenseId
...
async def test_get_group_settlements_success(expense_service, mock_group_data):
    ...
    mock_settlement_doc = {
        "_id": ObjectId(), "groupId": group_id, "payerId": "user_b", "payeeId": "user_c",
        "amount": 50.0, "status": "pending", "description": "A settlement",
        "createdAt": datetime.now(timezone.utc), "payerName": "User B", "payeeName": "User C"
    }
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.settlements.find.return_value.sort.return_value.skip.return_value.limit.return_value = mock_settlements_cursor
        ...
        mock_db.settlements.find.return_value.sort.return_value.skip.assert_called_with(0) # (1-1)*50
        mock_db.settlements.find.return_value.sort.return_value.skip.return_value.limit.assert_called_with(50)


@pytest.mark.asyncio
async def test_get_group_settlements_with_filters_and_pagination(expense_service, mock_group_data):
    ...
    mock_settlement_doc = {
        "_id": ObjectId(), "groupId": group_id, "payerId": "user_b", "payeeId": "user_c",
        "amount": 50.0, "status": "completed", "description": "A settlement",
        "createdAt": datetime.now(timezone.utc), "payerName": "User B", "payeeName": "User C"
    }
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_settlements_cursor = AsyncMock()
        mock_settlements_cursor.to_list.return_value = [mock_settlement_doc] * 5 # Simulate 5 settlements for this page
        mock_db.settlements.find.return_value.sort.return_value.skip.return_value.limit.return_value = mock_settlements_cursor
        mock_db.settlements.count_documents = AsyncMock(return_value=15) # Total 15 settlements matching filter

        result = await expense_service.get_group_settlements(group_id, user_id, status_filter=status_filter, page=page, limit=limit)
        ...
        mock_db.settlements.find.return_value.sort.return_value.skip.assert_called_with((page - 1) * limit)
        mock_db.settlements.find.return_value.sort.return_value.skip.return_value.limit.assert_called_with(limit)
...
async def test_get_settlement_by_id_success(expense_service, mock_group_data):
    ...
    mock_settlement_doc = {
        "_id": settlement_id_obj, "groupId": group_id, "payerId": "user_b",
        "payeeId": "user_c", "amount": 75.0, "status": "pending",
        "description": "Specific settlement", "createdAt": datetime.now(timezone.utc),
        "payerName": "User B", "payeeName": "User C"
    }
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        result = await expense_service.get_settlement_by_id(group_id, settlement_id_str, user_id)
        ...
        mock_db.settlements.find_one.assert_called_once_with({
            "_id": ObjectId(settlement_id_str),
            "groupId": group_id
        })
...
async def test_get_settlement_by_id_not_found(expense_service, mock_group_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.groups.find_one = AsyncMock(return_value=mock_group_data)
        mock_db.settlements.find_one = AsyncMock(return_value=None) # Settlement not found
...
async def test_get_settlement_by_id_group_access_denied(expense_service):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.groups.find_one = AsyncMock(return_value=None) # User not in group / group doesn't exist
...
async def test_update_settlement_status_success(expense_service):
    ...
    original_settlement_doc = {
        "_id": settlement_id_obj, "groupId": group_id, "status": "pending",
        "payerId": "p1", "payeeId": "p2", "amount": 10, "payerName": "P1", "payeeName": "P2",
    ...
    updated_settlement_doc["updatedAt"] = datetime.now(timezone.utc) # Will be set by the method
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        assert update_call_args[0] == {"_id": settlement_id_obj, "groupId": group_id} # Filter query
...
async def test_delete_settlement_success(expense_service, mock_group_data):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        result = await expense_service.delete_settlement(group_id, settlement_id_str, user_id)
...
async def test_get_user_balance_in_group_success(expense_service, mock_group_data):
    ...
    mock_pending_settlements_docs = [ # User B is payee, i.e. is owed
        {
            "_id": ObjectId(), "groupId": group_id, "payerId": "user_a", "payeeId": target_user_id_str,
            "amount": 100.0, "status": "pending", "description": "Owed to B",
            "createdAt": datetime.now(timezone.utc), "payerName": "User A", "payeeName": "User B Target"
        }
    ]
    mock_recent_expenses_docs = [ # Expense created by B, B also has a split
        {
            "_id": ObjectId(), "groupId": group_id, "createdBy": target_user_id_str,
            "description": "Lunch by B", "amount": 150.0,
            "splits": [{"userId": target_user_id_str, "amount": 75.0}, {"userId": "user_c", "amount": 75.0}],
            "createdAt": datetime.now(timezone.utc)
        }
    ]
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.settlements.find.return_value = mock_pending_cursor # This is the first .find() call
        ...
        mock_db.expenses.find.return_value.sort.return_value.limit.return_value = mock_expenses_cursor


        result = await expense_service.get_user_balance_in_group(group_id, target_user_id_str, current_user_id)
        ...
        assert result["owesYou"] is True # Net balance is positive, so target_user_id is owed money (by others in general)
        ...
        assert settlements_find_call_args["payeeId"] == target_user_id_str # For pending settlements
...
async def test_get_user_balance_in_group_access_denied(expense_service):
    ...
    group_id = str(ObjectId())
    target_user_id_str = str(ObjectId()) # Use a valid ObjectId string for target
    current_user_id = "user_x" # Not in group

    with patch('app.expenses.service.mongodb') as mock_mongodb:
        mock_db = MagicMock()
        mock_mongodb.database = mock_db
        mock_db.groups.find_one = AsyncMock(return_value=None) # Current user not member
...
async def test_get_friends_balance_summary_success(expense_service):
    ...
    group1_id = str(ObjectId()) # Remains as string, used for direct comparison in mock
    ...
    mock_groups_data = [
        {
            "_id": ObjectId(group1_id), "name": "Group Alpha",
            "members": [{"userId": user_id_str}, {"userId": friend1_id_str}]
        },
        {
            "_id": ObjectId(group2_id), "name": "Group Beta",
            "members": [{"userId": user_id_str}, {"userId": friend1_id_str}, {"userId": friend2_id_str}]
        }
    ]
    ...
    def sync_mock_settlements_aggregate_cursor_factory(pipeline, *args, **kwargs):
        ...
        if group_id_pipeline == group1_id and pipeline_friend_id == friend1_id_str:
            ...
            mock_agg_cursor.to_list.return_value = [{"_id": None, "userOwes": 50.0, "friendOwes": 0.0}]
        elif group_id_pipeline == group2_id and pipeline_friend_id == friend1_id_str:
            # Friend1 owes Main 30 in Group Beta
            mock_agg_cursor.to_list.return_value = [{"_id": None, "userOwes": 0.0, "friendOwes": 30.0}]
        elif group_id_pipeline == group2_id and pipeline_friend_id == friend2_id_str:
            # Main owes Friend2 70 in Group Beta
            mock_agg_cursor.to_list.return_value = [{"_id": None, "userOwes": 70.0, "friendOwes": 0.0}]
        else:
            mock_agg_cursor.to_list.return_value = [{"_id": None, "userOwes": 0.0, "friendOwes": 0.0}] # Default empty
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        def mock_user_find_cursor_side_effect(query, *args, **kwargs):
            ids_in_query = query["_id"]["$in"] # These are already ObjectIds from the service
            users_to_return = []
            if friend1_id_obj in ids_in_query: users_to_return.append(mock_friend1_doc)
            if friend2_id_obj in ids_in_query: users_to_return.append(mock_friend2_doc)
        ...
        assert abs(friend1_summary["netBalance"] - (-20.0)) < 0.01 # Main owes Friend1 20
        ...
        assert abs(friend2_summary["netBalance"] - (-70.0)) < 0.01 # Main owes Friend2 70
...
async def test_get_friends_balance_summary_no_friends_or_groups(expense_service):
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        mock_db.users.find = MagicMock(return_value=mock_user_find_cursor) # find is sync, returns async cursor

        mock_db.settlements.aggregate = AsyncMock() # Won't be called if no friends/groups
...
async def test_get_overall_balance_summary_success(expense_service):
    ...
    mock_groups_data = [
        {"_id": ObjectId(group1_id), "name": "Group One", "members": [{"userId": user_id}]},
        {"_id": ObjectId(group2_id), "name": "Group Two", "members": [{"userId": user_id}]},
        {"_id": ObjectId(group3_id), "name": "Group Three", "members": [{"userId": user_id}]}
    ]
    ...
    def mock_aggregate_cursor_side_effect(pipeline, *args, **kwargs):
        ...
        if group_id_pipeline == group1_id:
            ...
            cursor_mock.to_list = AsyncMock(return_value=[{"_id": None, "totalPaid": 50.0, "totalOwed": 150.0}])
        elif group_id_pipeline == group3_id: # Zero balance
            cursor_mock.to_list = AsyncMock(return_value=[{"_id": None, "totalPaid": 50.0, "totalOwed": 50.0}])
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        group1_summary = next(g for g in result["groupsSummary"] if g["group_id"] == group1_id)
        group2_summary = next(g for g in result["groupsSummary"] if g["group_id"] == group2_id)
...
async def test_get_group_analytics_success(expense_service, mock_group_data):
    ...
    mock_expenses_in_period = [
        {
            "_id": ObjectId(), "groupId": group_id_str, "createdBy": user_a_str,
            "description": "Groceries", "amount": 70.0, "tags": ["food", "household"],
            "splits": [{"userId": user_a_str, "amount": 35.0}, {"userId": user_b_str, "amount": 35.0}],
            "createdAt": expense1_date
        },
        {
            "_id": ObjectId(), "groupId": group_id_str, "createdBy": user_b_str,
            "description": "Movies", "amount": 30.0, "tags": ["entertainment", "food"],
            "splits": [{"userId": user_a_str, "amount": 15.0}, {"userId": user_b_str, "amount": 15.0}],
    ...
    async def mock_users_find_one_side_effect(query, *args, **kwargs):
        ...
        if user_id_query_obj == user_a_obj: return mock_user_a_doc_db
        if user_id_query_obj == user_b_obj: return mock_user_b_doc_db
        if user_id_query_obj == user_c_obj: return mock_user_c_doc_db
        return None
    ...
    current_test_mock_group_data = {
        "_id": ObjectId(group_id_str), # Use the same ObjectId as in the service call
    ...
    with patch('app.expenses.service.mongodb') as mock_mongodb:
        ...
        # Mock group membership check
        mock_db.groups.find_one = AsyncMock(return_value=current_test_mock_group_data) # Use the adjusted mock
        ...
        result = await expense_service.get_group_analytics(group_id_str, user_a_str, period="month", year=year, month=month)
        ...
        entertainment_cat = next(c for c in top_categories if c["tag"] == "entertainment")

        assert abs(food_cat["amount"] - 100.0) < 0.01 and food_cat["count"] == 2
        assert abs(household_cat["amount"] - 70.0) < 0.01 and household_cat["count"] == 1
        assert abs(entertainment_cat["amount"] - 30.0) < 0.01 and entertainment_cat["count"] == 1
        ...
        day5_trend = next(d for d in result["expenseTrends"] if d["date"] == f"{year}-{month:02d}-05")
        assert abs(day5_trend["amount"] - 70.0) < 0.01 and day5_trend["count"] == 1
        day15_trend = next(d for d in result["expenseTrends"] if d["date"] == f"{year}-{month:02d}-15")
        assert abs(day15_trend["amount"] - 30.0) < 0.01 and day15_trend["count"] == 1
        day10_trend = next(d for d in result["expenseTrends"] if d["date"] == f"{year}-{month:02d}-10") # No expense
        ...
        assert mock_db.users.find_one.call_count == len(current_test_mock_group_data["members"])
