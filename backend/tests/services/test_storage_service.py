...
def fixed_uuid(monkeypatch):
    monkeypatch.setattr("app.services.storage.uuid4", lambda: UUID("12345678123456781234567812345678"))
    yield
...
async def test_validate_file_valid_png(monkeypatch):
    ...
    # Patch limits
    monkeypatch.setattr("app.services.storage.settings", type("obj", (), {"MAX_FILE_SIZE": 5*1024*1024}))
    monkeypatch.setattr("app.services.storage.settings", type("obj", (), {"MAX_FILE_SIZE": 5*1024*1024, "MAX_IMAGE_PIXELS": 10000}))
...
async def test_save_to_quarantine_uploadfile(monkeypatch):
    ...
    monkeypatch.setattr(service, "generate_secure_file_path", lambda *args, **kwargs: "folder/entity/file.webp")
...
async def test_save_to_quarantine_bytes(monkeypatch):
    ...
    m_open = mock_open()
    with patch("builtins.open", m_open):
        result = await service.save_to_quarantine(file_content, "folder", "entity", "file.webp")
...
async def test_save_to_quarantine_invalid_type(monkeypatch):
    ...
    monkeypatch.setattr(service, "generate_secure_file_path", lambda *a, **kw: "folder/entity/file.webp")
...
async def test_save_to_quarantine_read_failure(monkeypatch):
    ...
    monkeypatch.setattr(service, "generate_secure_file_path", lambda *a, **kw: "folder/entity/file.webp")
    monkeypatch.setattr("app.services.storage.BASE_QUARANTINE_DIR", MagicMock(spec=Path))
...
async def test_upload_to_firebase_signed_url(monkeypatch):
    ...
    monkeypatch.setattr(storage_module.storage, "bucket", lambda name: mock_bucket)
    monkeypatch.setattr(service, "get_bucket", lambda: "bucket_name")
    monkeypatch.setattr(service, "generate_signed_url", AsyncMock(return_value="http://signed-url.com"))

    urls = await service.upload_to_firebase(processed_images, "file/path")

    assert urls["thumbnail"] == "http://signed-url.com"
    mock_blob.upload_from_string.assert_called_once_with(b"thumb", content_type="image/webp")
    service.generate_signed_url.assert_awaited_once_with(mock_blob, expires_seconds=storage_module.SIGNED_URL_EXPIRY_SECONDS)
...
async def test_upload_to_firebase_generate_signed_url_failure(monkeypatch):
    ...
    monkeypatch.setattr(storage_module.storage, "bucket", lambda name: mock_bucket)
    monkeypatch.setattr(service, "get_bucket", lambda: "bucket_name")
    monkeypatch.setattr(service, "generate_signed_url", AsyncMock(side_effect=Exception("signed_url fail")))
...
async def test_move_quarantine_to_public_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(service, "process_image", AsyncMock(return_value={"thumbnail": b"thumb", "full": b"full"}))
    monkeypatch.setattr(service, "upload_to_firebase", AsyncMock(return_value={"thumbnail": "url_thumb", "full": "url_full"}))
...
async def test_move_quarantine_to_public_file_missing(monkeypatch):
    ...
    with pytest.raises(ValueError, match="Quarantine file not found"):
        await service.move_quarantine_to_public("nonexistent_file.webp", "public/folder/path")
...
async def test_move_quarantine_to_public_process_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(service, "process_image", AsyncMock(side_effect=RuntimeError("Processing failed")))
    
    with pytest.raises(RuntimeError, match="Processing failed"):
        await service.move_quarantine_to_public(str(quarantine_file), "public/folder/path")
...
async def test_move_quarantine_to_public_upload_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(service, "process_image", AsyncMock(return_value={"thumbnail": b"thumb"}))
    monkeypatch.setattr(service, "upload_to_firebase", AsyncMock(side_effect=RuntimeError("Upload failed")))
...
async def test_process_image_success(monkeypatch):
    ...
    monkeypatch.setattr("app.services.storage.process_image", AsyncMock(return_value={
        "thumbnail": b"thumb",
        "medium": b"medium",
        "full": b"full"
    }))
...
async def test_process_image_failure(monkeypatch):
    ...
    monkeypatch.setattr("app.services.storage.process_image", AsyncMock(side_effect=Exception("Some error")))
...
async def test_delete_image_success(monkeypatch):
    ...
    expected_calls = [f"folder/file_{size}.webp" for size in ["thumbnail", "medium", "full"]]
...
async def test_upload_image_workflow_success(monkeypatch, tmp_path):
    ...
    dummy_file = UploadFile(filename="image.png", file=tmp_path.joinpath("dummy").open("wb+"))
    ...
    monkeypatch.setattr(service, "save_to_quarantine", AsyncMock(return_value=str(tmp_path / "quarantine_file.webp")))
    monkeypatch.setattr(service, "move_quarantine_to_public", AsyncMock(return_value={"thumbnail": "url_thumb"}))
    monkeypatch.setattr(service, "generate_secure_file_path", lambda entity_id, folder, filename: f"{folder}/{entity_id}/secure_id.png")
...
async def test_upload_image_workflow_processing_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(service, "validate_file", AsyncMock(return_value=True))
    monkeypatch.setattr(service, "save_to_quarantine", AsyncMock(return_value=str(tmp_path / "quarantine_file.webp")))
    monkeypatch.setattr(service, "move_quarantine_to_public", AsyncMock(side_effect=RuntimeError("Upload failed")))
    monkeypatch.setattr(service, "generate_secure_file_path", lambda entity_id, folder, filename: f"{folder}/{entity_id}/secure_id.png")
