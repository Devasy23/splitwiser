...
try:
    ...
    if not use_emulator and (not service_account_path or not os.path.exists(service_account_path)):
...
class FirebaseStorageService:
    ...
    def generate_secure_file_path(self, entity_id: str, folder: str, filename: str) -> str:
        """
    ...
    async def validate_file(self, file: UploadFile) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Pillow failed to fully load the image (possibly corrupt or too large).")
                return False

            img_format = img.format
            if img_format is None or PIL_FORMAT_TO_MIME.get(img_format) not in ["image/jpeg", "image/png", "image/webp"]:
    ...
    async def save_to_quarantine(self,file: Union[UploadFile, bytes],folder: str,
                                 entity_id: str,original_filename: str) -> str:
        """
    ...
    async def generate_signed_url(self, blob, expires_seconds: int = SIGNED_URL_EXPIRY_SECONDS) -> str:
        """
    ...
    async def upload_to_firebase(self, processed_images: Dict[str, bytes], file_path: str) -> Dict[str, str]:
        """
        ...
        try:
            ...
            for size, content in processed_images.items():
                ...
                if use_emulator:
                    ...
                    url = await self.generate_signed_url(blob, expires_seconds=SIGNED_URL_EXPIRY_SECONDS)
    ...
    async def move_quarantine_to_public(self, quarantine_path: str, public_base_path: str) -> Dict[str, str]:
        """
    ...
    async def process_image(self, file_content: bytes) -> Dict[str, bytes]:
        ...
        """
        try:
            return await process_image(file_content) # Calls image_proccessor.py's process_image
        except Exception as e:
    ...
    async def upload_image_workflow(self, file: UploadFile, folder: str, entity_id: str) -> Dict[str, str]:
        """
        Workflow:
        ...
        try:
            ...
            quarantine_path = await self.save_to_quarantine(content, folder, entity_id, file.filename)

            if CLAMAV_ENABLED:
                # Original path: scan + process inline
                urls = await self.move_quarantine_to_public(quarantine_path, file_path)
                logger.info(f"Upload workflow successful for {folder} entity {entity_id}")
                return urls
            else:
