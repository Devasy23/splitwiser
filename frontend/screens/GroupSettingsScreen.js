  ...
  const renderMemberItem = (m) => {
    ...
    return (
      <List.Item
        ...
        right={() => (
          isAdmin && !isSelf ? (
            <IconButton icon="account-remove" onPress={() => onKick(m.userId, displayName)} />
          ) : null
        )}
      />
    );
    ...
    <View style={styles.container}>
      <Card style={styles.card}>
        ...
        <Card.Content>
          ...
          <View style={styles.iconRow}>
            {ICON_CHOICES.map(i => (
              <Button key={i} mode={icon === i ? 'contained' : 'outlined'} style={styles.iconBtn} onPress={() => setIcon(i)}>
                {i}
              </Button>
            ))}
          </View>
          ...
          />
          {isAdmin && (
            <Button mode="contained" style={{ marginTop: 12 }} loading={saving} disabled={saving} onPress={onSave}>
              Save Changes
            </Button>
          )}
        </Card.Content>
      ...
      <Card style={styles.card}>
        ...
        <Card.Content>
          ...
          <Button mode="outlined" onPress={onShareInvite} icon="share-variant">Share invite</Button>
        </Card.Content>
      </Card>
      ...
      <Card style={styles.card}>
        ...
        <Card.Content>
          <View style={{ gap: 8 }}>
            <Button mode="outlined" buttonColor="#fff" textColor="#d32f2f" onPress={onLeave} icon="logout-variant">
              Leave Group
            </Button>
            {isAdmin && (
              <Button mode="contained" buttonColor="#d32f2f" onPress={onDeleteGroup} icon="delete">
                Delete Group
              </Button>
            )}
          </View>
